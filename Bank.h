	...
	*/
	std::vector<int> find_accounts_by_name(std::string name)
	{
		...
		for(int i =0;i<accounts.size();i++)
		{
			...
			if (name ==acc->get_customer->getname())
			{
...
public:
	...
	*/
		...
		std::string cust_type, std::string account_type)
	{
		...
		if (cust_type == "Senior")
		{
			cust = new Senior(name,telephone,age,address,customer_id); 
		}
		else if (cust_type == "Adult")
		{
			cust = new Adult(name, telephone, age, address, customer_id);

		}
		else
		{
			cust = new student(name, telephone, age, address, customer_id);
